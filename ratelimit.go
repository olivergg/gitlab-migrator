...
func newRateLimitedTransport(base http.RoundTripper) *rateLimitedTransport {
	...
	return &rateLimitedTransport{
		base:           base,
		coreLimiter:    rate.NewLimiter(rate.Limit(0.5), 1),  // 0.33 req/sec (3 per 10 sec) for reads
		searchLimiter:  rate.NewLimiter(rate.Limit(0.2), 1),  // 0.1 req/sec (1 per 10 sec) for search
		writeLimiter:   rate.NewLimiter(rate.Limit(0.2), 1), // 0.033 req/sec (1 every ~30 sec) - very conservative for content creation
		logger:         logger,
	}
}
